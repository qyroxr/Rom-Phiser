...
CONTAINER="romphisher"
IMAGE="qyroxr/romphisher:latest"
IMG_MIRROR="ghcr.io/qyroxr/romphisher:latest"
...
if [[ ! $check_container == $CONTAINER ]]; then
	...
	docker create \
		--interactive --tty \
		--volume ${MOUNT_LOCATION}:/romphisher/auth/ \
		--network host \
		--name "${CONTAINER}" \
		"${IMAGE}"
fi
...
# docker run --rm -ti --network="host" -v ${MOUNT_LOCATION}:/romphisher/auth/ --name "$CONTAINER" "$IMAGE"
